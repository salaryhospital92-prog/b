...
import { CALL_PRICES, calculateCallTotal } from "../lib/rules-engine";
...
export default function ResidentWorkflow({ role, accountName, mode = "logs" }: { role: WorkflowRole; accountName: string; mode?: "logs" | "objections" }) {
  ...
  return <>
    ...
    {editorOpen && <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && setEditorOpen(false)}><form className="entry-modal workflow-modal" key={editing?.id || "new"} onSubmit={submitLog}>
      <div className="modal-head"><div><p className="eyebrow">{editing ? `تعديل مسجل باسم ${accountName}` : "سجل طبيب مقيم جديد"}</p><h2>{editing ? `تحديث مناوبة ${editing.doctorName}` : "تسجيل مناوبة اليوم"}</h2></div><button type="button" className="close-button" onClick={() => setEditorOpen(false)}>×</button></div>
      <div className="workflow-fields"><label className="form-field"><span>اسم صاحب السجل</span><input value={editing?.doctorName || accountName} disabled /></label><label className="form-field"><span>تاريخ اليوم</span><input name="callDate" type="date" required defaultValue={editing?.callDate || today()} /></label><label className="form-field"><span>مدة الكول</span><select name="callHours" defaultValue={String(editing?.callHours || 12)}><option value="12">12 ساعة</option><option value="24">24 ساعة</option></select></label><label className="form-field"><span>عدد الرقود</span><input name="inpatients" type="number" min="0" required defaultValue={editing?.inpatients || 0} /></label><label className="form-field"><span>عدد الولادات</span><input name="births" type="number" min="0" required value={birthCount} onChange={(event) => setBirthCount(Math.max(0, Number(event.target.value) || 0))} /></label><label className="form-field"><span>عدد الحالات الخاصة</span><input name="specialCases" type="number" min="0" required value={specialCount} onChange={(event) => setSpecialCount(Math.max(0, Number(event.target.value) || 0))} /></label><label className="form-field full"><span>تفاصيل الحالات الخاصة</span><textarea name="specialNote" rows={2} defaultValue={editing?.specialNote || ""} placeholder="معلومة سريرية موجزة لا تتضمن بيانات حساسة غير لازمة" /></label><label className="form-field"><span>عدد الاستشاريات</span><input type="number" min="0" required value={consultationCount} onChange={(event) => setConsultationCount(Math.max(0, Number(event.target.value) || 0))} /></label></div>
      ...
      <div className="calculation-note"><span>₫</span><div className="call-preview"><b>حساب الكول قبل التدقيق</b><ul>
        <li><span>الاستشاريات × {IQD.format(consultationCount)}</span><i>{money(consultationCount * CALL_PRICES.consultation)}</i></li>
        <li><span>الولادات × {IQD.format(birthCount)}</span><i>{money(birthCount * CALL_PRICES.birth)}</i></li>
        <li className="call-preview-note"><span>الحالات الخاصة × {IQD.format(specialCount)}</span><i>للتوثيق</i></li>
        <li className="call-preview-total"><span>مجموع الكول</span><i>{money(calculateCallTotal({ consultations: consultationCount, births: birthCount }))}</i></li>
      </ul><small>حساب الكول وحده قبل تطبيق السقوف. الحالات الخاصة تُوثَّق ولا تُسعَّر، وأيام الرقود تُحتسب يوميًا في «أيام الرقود» بعد تأكيد الدفع.</small></div></div>
      <div className="modal-actions"><button type="button" className="secondary-action" onClick={() => setEditorOpen(false)}>إلغاء</button><button type="submit" className="primary-action" disabled={saving}>{saving ? "جارٍ الحفظ..." : editing ? "حفظ التعديل" : "حفظ السجل"}</button></div>
    </form></div>}
